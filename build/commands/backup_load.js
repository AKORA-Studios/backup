...
module.exports = new classes_1.Command({
    ...
    utilities_1.getFile(message, "Please send me your backup file", 30000, (obj) => {
        ...
        utilities_1.confirmAction(message, "Please Confirm you want to load this Backup", async () => {
            var emb = utilities_1.newEmb(message).setColor(utilities_1.colors.success).setTitle("Loading Backup"), text = "", msg = await message.channel.send(emb), send = msg.edit;
            ...
            msg.channel.send(utilities_1.newEmb(message).setColor(utilities_1.colors.success).setTitle("Finished Loading uwu").setFooter("Took: " + span + " seconds"));
        }, () => {
        });
    }, () => {
    });
